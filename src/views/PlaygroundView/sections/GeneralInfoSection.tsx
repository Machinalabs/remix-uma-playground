    ...
    return (
        <Container fluid={true}>
            ...
            <Row>
                <StyledCol style={{ paddingRight: "1em" }}>
                    <h5>General Info</h5>
                    <p>Expire date: <span>{expireDate}</span></p>
                    <p>Is expired: <span>{isExpired}</span></p>
                    <p>Price identifier: <span>{priceIdentifier}</span></p>
                    <p>Global collateral ratio: <span>{gcr}</span></p>
                    <p>Minimum sponsor tokens: <span>{minimunSponsorTokens}</span></p>

                </StyledCol>
                <StyledCol style={{ paddingLeft: "1em" }}>
                    <h5>Dispute Params</h5>
                    <p>Withdrawal liveness in min: <span>{withdrawalLiveness}</span></p>
                    <p>Liquidation liveness in min: <span>{liquidationLiveness}</span></p>
                </StyledCol>
            </Row>
        </Container>
    )
